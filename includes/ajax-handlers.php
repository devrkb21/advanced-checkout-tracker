...
function act_handle_check_courier_success()
{
    ...
    $phone_number = isset($_POST['phone_number']) ? act_normalize_phone_number(sanitize_text_field($_POST['phone_number'])) : '';
...
function act_handle_save_checkout_data()
{
    ...
    $data_to_save = [
        ...
        'phone' => isset($posted_data['billing_phone']) ? act_normalize_phone_number($posted_data['billing_phone']) : '',
    ...
    <div class="act-redesign-container">
        ...
        <div class="act-redesign-right">
            ...
            <div class="act-powered-by">
                ...
                <span class="community-link">
                    Join Our <a href="https://chat.whatsapp.com/JGUTBCNqK7d32zHWOQ4wzR" target="_blank">WhatsApp
                        Community</a>
                </span>
            </div>
        </div>
...
function act_live_ratio_check_handler()
{
    ...
    $phone_number = act_normalize_phone_number(sanitize_text_field($_POST['phone_number']));
...
/**
 * Decrements the usage count for a specific action via API call to HQ.
 *
 * @param string $action_type 'checkouts', 'fraud_blocks', etc.
 */
function act_decrement_usage_count($action_type)
{
    // Immediately update the local transient for a faster UI response.
    $config = get_transient('act_api_cache');
    if ($config && isset($config['usage'][$action_type]) && $config['usage'][$action_type] > 0) {
        $config['usage'][$action_type]--;
        set_transient('act_api_cache', $config, DAY_IN_SECONDS);
    }

    // Send the decrement request to the HQ server.
    act_call_hq_api('/sites/decrement-usage', ['action_type' => $action_type]);
}

/**
 * Handles the AJAX request to get blocked order log details.
 */
function act_get_blocked_order_details_ajax_handler()
{
    check_ajax_referer('act_view_details_nonce', 'nonce');
    if (!current_user_can('manage_woocommerce')) {
        wp_send_json_error(['message' => __('Permission denied.', 'advanced-checkout-tracker')]);
    ...
    if (!isset($_POST['entry_id']) || !is_numeric($_POST['entry_id'])) {
        wp_send_json_error(['message' => __('Invalid ID provided.', 'advanced-checkout-tracker')]);
        return;
    }
    $entry_id = intval($_POST['entry_id']);
    ...
    if (!$entry) {
        wp_send_json_error(['message' => __('Blocked order log not found.', 'advanced-checkout-tracker')]);
        return;
    }
    ...
    ?>
        <div class="act-entry-details">
            ...
            <p><strong><?php _e('Email Address:', 'advanced-checkout-tracker'); ?></strong>
                <?php echo esc_html($entry->email_address); ?></p>
            <p><strong><?php _e('IP Address:', 'advanced-checkout-tracker'); ?></strong>
                <?php echo esc_html($entry->ip_address); ?></p>
            <hr>
            <h3><?php _e('Block Details', 'advanced-checkout-tracker'); ?></h3>
            <p><strong><?php _e('Blocked At:', 'advanced-checkout-tracker'); ?></strong>
                <?php echo esc_html(date_i18n(get_option('date_format') . ' ' . get_option('time_format'), strtotime($entry->blocked_at))); ?>
            </p>
            <p><strong><?php _e('Success Ratio at Block:', 'advanced-checkout-tracker'); ?></strong> <span
                    style="color: red; font-weight: bold;"><?php echo esc_html($entry->success_ratio); ?>%</span></p>
            <p><strong><?php _e('Threshold at Block:', 'advanced-checkout-tracker'); ?></strong>
                <?php echo esc_html($entry->threshold_at_block); ?>%</p>
            <hr>
            <h3><?php _e('Cart Details at Time of Block', 'advanced-checkout-tracker'); ?></h3>
            <p><strong><?php _e('Cart Value:', 'advanced-checkout-tracker'); ?></strong>
            ...
            if (is_array($cart_items) && !empty($cart_items)) {
                ...
                echo '<p>' . __('No cart items found.', 'advanced-checkout-tracker') . '</p>';
            }
            ?>
        </div>
        <?php
